...
class Interface {
  ...
  generateInstallIteratorPrototype() {
    ...
    if (iterable.isAsync) {
      this.requires.addDeclaration("newObjectInRealm", "utils.newObjectInRealm");
      this.str += `
  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  generateRequires() {
    this.requires.addDeclaration("implSymbol", "utils.implSymbol");
    this.requires.addDeclaration("ctorRegistrySymbol", "utils.ctorRegistrySymbol");

    if (this.idl.inheritance !== null) {
      this.requires.addRelative(this.idl.inheritance);
      this.requires.addDeclaration("inheritance", `"${this.idl.inheritance}"`);
      this.exports.push("inheritance");
    }
  ...
  generateExport() {
    ...
    if (this.iterable) {
      if (this.iterable.isAsync) {
        this.exports.push("createDefaultAsyncIterator");
        this.str += `
        ...
        this.exports.push("createDefaultIterator");
        this.str += `
  ...
  addConstructor() {
    ...
    if (overloads.length !== 0) {
      ...
      body = `
        ${conversions.body}
        return setup(${utils.formatArgs(setupArgs)});
      `;
    } else {
      body = `
  ...
  generate() {
    ...
    this.str += `
      export default { ${this.exports.join(", ")}};
    `;
  }
